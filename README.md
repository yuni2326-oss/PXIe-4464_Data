...
- **다채널 동시 수집 (최대 12채널)**
  ...
  - PXIe-4492: 8채널, **내부 IEPE 구동** (`PXI1Slot5`, 선택 사용) — 채널별로 가속도계/마이크 지정
  - 채널별 개별 활성화/비활성화
- **혼합 센서 지원**: 가속도계(→ m/s²)와 마이크로폰(→ Pa)을 채널별로 구분해 각각 올바른 물리단위로 취득
  - 예: CH6에 Crysound 333T1(IEPE, 50 mV/Pa) 연결 → 캐비테이션 소음 측정
- **실시간 시각화**: 시간 파형 + FFT 스펙트럼 (pyqtgraph, **표시 상한 UI 설정** — 진동 5kHz / 음향 20kHz 등)
- **이상 감지**: Z-score + **정규화 마할라노비스 거리** 기반
  `LEARNING → NORMAL → WARNING → ALARM` (홀드오프 3회)
- **주기적 자동 저장 + NAS 전송**: raw `.npz` + FFT CSV를 로컬 저장 후 백그라운드로 NAS 복사
- **무인 운전 안정화**
  - DAQ 오류 시 **초기 실행조건 그대로 자동 재연결·재시작** (지수 백오프) + 외부 supervisor
  - 멈춘 버퍼 반복 저장 방지(stale 가드), 비차단 오류 알림
  - 회전 파일 로그 + heartbeat로 상태 추적
...
|------|-----------|-----------|
| 종류 | Sound & Vibration (IEPE) | Sound & Vibration (IEPE) |
| 기본 슬롯 | `PXI1Slot3` | `PXI1Slot5` |
| 채널 | 4 (ai0~ai3) | 8 (ai0~ai7) |
| 센서 | IEPE 가속도계 (내부 4 mA) | IEPE 가속도계·마이크 혼용 (내부 4 mA) |
| 채널 함수 | `add_ai_accel_chan` | `add_ai_accel_chan` / `add_ai_microphone_chan` |
| 최대 샘플레이트 | 204.8 kS/s | 204.8 kS/s |
| ADC | 24-bit | 24-bit |

> **CH ↔ 물리 채널 매핑**: CH0–3 = 4464 ai0–3, CH4–11 = 4492 ai0–7.
> 예) CH6 = 4492 ai2. "마이크 채널"에 전역 CH 번호(예: `6`) 입력 시 해당 채널만 마이크(Pa)로 취득.
...
DAQ(연속) → AcquisitionWorker(백그라운드 스레드)
   → FeatureCollector(rolling buffer, 주기적 추출)
        ...
        └→ raw 윈도우 → DataSaver(raw .npz + FFT CSV) → NAS 백그라운드 전송
```
...
2. **센서 설정**: 가속도계 감도, (마이크 사용 시) 마이크 채널·감도·최대 SPL — 아래 "센서 설정" 참고
3. 샘플레이트·수집 주기·수집 시간·학습 누적 횟수·저장 주기·채널·FFT 표시 상한 설정
4. **오류 시 자동 재시작** 체크(기본 ON) — 무인 운전 권장
5. **연결** → **▶ 시작**

### 센서 설정 (가속도계 / 마이크)

두 장비 모두 **내부 IEPE**로 구동하며, 드라이버가 물리단위로 직접 변환해 반환한다.

- **가속도계 채널**(4464 전체 + 4492의 마이크 외 채널): `add_ai_accel_chan`
  - "가속도계 감도"(mV/(m/s²), 기본 10.2 = PCB 352C33) 입력 → **m/s²** 출력
- **마이크 채널**(4492 중 "마이크 채널"로 지정한 CH): `add_ai_microphone_chan`
  - "마이크 감도"(mV/Pa, 기본 50 = Crysound 333T1), "마이크 최대 SPL(dB)" 입력 → **Pa** 출력

관련 UI 필드:

| 필드 | 기본값 | 설명 |
|------|--------|------|
| 가속도계 감도 (mV/(m/s²)) | 10.2 | 가속도계 채널 공통 |
| 마이크 채널 (CH) | (없음) | 마이크로 취급할 전역 CH 번호. 예: `6` |
| 마이크 감도 (mV/Pa) | 50 | 마이크 채널 감도 |
| 마이크 최대 SPL (dB) | 130 | 감도와 조합해 입력 레인지 결정. 너무 높으면 장비 오류(-200860) → 낮출 것 |
| FFT 표시 상한 (Hz) | 5000 | 표시 X축 상한. 음향(캐비테이션)은 20000 권장 |

> **마이크 최대 SPL 주의**: `필요 전압 = 10^(SPL/20)·20µPa·감도`. 50 mV/Pa에서 140 dB→10 V(±10V 필요, IEPE 입력 범위 초과로 거부),
> **130 dB→3.16 V(±5V 이내 OK)**. 클리핑되면 올리되 134 dB(≈5 V) 이하로 유지.

### 저장 형식 및 NAS 전송

- **raw**: `results/<타임스탬프>_raw.npz` (float32, 단위 m/s²)
  읽기: `d = numpy.load("..._raw.npz"); arr = d["data"]  # (채널, 샘플); sr = float(d["sample_rate"])` — 시간축 `t[i]=i/sr`
- **FFT**: `results/<타임스탬프>_fft_ch{n}.csv` (전대역)
- **저장 하위폴더**: UI에 이름 입력 시 `results/<이름>/`에 분리 저장 (비우면 루트)
- **NAS 전송**: "NAS 전송 경로" 지정 시 로컬 저장 직후 **백그라운드로 NAS에 복사**
  (기본 `\\10.130.121.158\ai_gpu_001\GY_DATA\pump data`, 하위폴더 구조 미러링).
  네트워크 지연·끊김은 백그라운드 처리라 수집에 영향 없음. 실패 시 3회 재시도 후 로컬 보존.
  "NAS 전송 후 로컬 삭제" 체크 시 전송 성공한 파일만 로컬에서 삭제.
- 저장 주기 `0`분 = 매 사이클 저장.

> 5초 윈도우·6채널·51.2kS/s 기준 로컬 쓰기 ~2초(raw npz + FFT 전대역 CSV). 코스트다운
> 연속 저장(수집 주기 5초/저장 0)에 여유가 있다. FFT까지 0–5kHz로 줄이면 ~0.5초.
...
| 4492 채널 값 4464 대비 5~6배 약함 | 4492를 전압채널로 잡아 **IEPE 미공급** (센서는 IEPE) | 4492를 accel/mic 채널 + **내부 IEPE**로 전환 → 레벨 정상화(≈10배↑) |
| 4492 마이크 태스크 생성 오류(-200860) | 최대 SPL 140 dB가 IEPE 입력 범위(±5V) 초과 | 최대 SPL UI 지정(기본 130 dB=3.16V) |
