...
- **raw**: `results/<타임스탬프>_raw.csv` (컬럼: time_s, ch0..chN-1, 단위 m/s²)
- **FFT**: `results/<타임스탬프>_fft_ch{n}.csv` (전대역)
- **저장 하위폴더**: UI에 이름 입력 시 `results/<이름>/`에 분리 저장 (비우면 루트)
- 저장 주기 `0`분 = 매 사이클 저장.

> 5초 윈도우·6채널·51.2kS/s 기준 CSV 쓰기는 **~4초**가 걸린다. 코스트다운처럼 매 사이클
> 연속 저장하려면 수집 주기를 쓰기 시간보다 넉넉히(예: 8~10초) 잡거나, 쓰기가 빠른
> `.npz` 저장 방식을 사용한다(태그 `460c86f` 이후 버전 / 별도 요청 시 재적용).
